...
if ($isMember) {
    Write-Log "$($user.SamAccountName) is already a member of '$($group.Name)'." "Yellow"
    exit 0
}

try {
    Add-ADGroupMember -Identity $GroupName -Members $Username -ErrorAction Stop
    Write-Log "Added '$($user.SamAccountName)' to '$($group.Name)'." "Green"
} catch {
    Write-Log "Failed to add to group: $($_.Exception.Message)" "Red"; exit 1
}
